    ...
    const cartbody={
        ...
        const handleIncreaseQuantity = () => {
          ...
          const addToCart = async () => {
            try {
              const response = await axios.post(`http://localhost:8000/cart/user/${localStorage.getItem("user")}`, {
                headers: {
                        ...
                        <Col key={products.products.productId} xs={12} className="mb-3">
                            <Card style={{ width: '100%', backgroundColor: 'black', border: '1px solid #333' }}>
                                <Row noGutters>
                                    ...
                                    <Col xs={6}>
                                        <Card.Body>
                                            <Card.Title style={{ fontWeight: 'bold', fontSize: '1.5rem', color: '#ad8cd4' }}>{products.products.productName}</Card.Title>
                                            <Card.Text style={{ color: '#fff' }}>Breed : {products.products.description.breed}</Card.Text>
